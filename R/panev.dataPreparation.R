  ...
  )
{
  ...
  if (gene_id=="ensembl"){
    ...
    cat("n.", paste(sum(length(which(!is.na(genelist$entrezgene))))), "of", paste(sum(length(which(!is.na(genelist$ensembl_gene_id))))), "genes have corresponding gene in KEGG database.", sep= " ")
    wd <- getwd()
    ...
    cat("DONE\n")
    #cat("Checking for corresponding genes in KEGG database ... \n")
    cat("n.", paste(sum(length(which(!is.na(genelist$ensembl_gene_id))))), "of", paste(sum(length(which(!is.na(genelist$entrezgene))))), "genes have corresponding gene in KEGG database.", sep= " ")
    write.table(x = genelist, file = paste(in.file,"_converted.txt",sep=""), row.names = F)
    cat("\n")
    cat("Gene list exported!")
    return(genelist)
